...
TESPathGridPoint* TESRender::PickPathGridPointAtCoords(int X, int Y)
{
	...
	if (TESRenderWindow::PickBuffer->PerformPick(&RayOrigin, &RayDir))
	{
		if (TESRenderWindow::PickBuffer->pickRecords.numObjs > 0)
		{
			...
			{
<<<<<<< HEAD
				NiIntegerExtraData* xData = NI_CAST(GetExtraData(Pick->m_parent, "INDEX"), NiIntegerExtraData);
				if (xData)
					Result = (TESPathGridPoint*)xData->m_iValue;
=======
				NiAVObject* Parent = NI_CAST(Pick->m_parent, NiAVObject);
				if (Parent)
				{
					NiIntegerExtraData* xData = NI_CAST(GetExtraData(Parent, "INDEX"), NiIntegerExtraData);
					if (xData)
						Result = (TESPathGridPoint*)xData->m_iValue;
				}
